...
do
    ...
    then  # if "high" then show highest value (tail)
            cut -f "$1" $gapminderfile | sort -n | tail -1 >> gm_field"$1"_"$2".out #capture output into a file
    else  # otherwise show lowest value (head)
            cut -f "$1" $gapminderfile | sort -n | head -1 >> gm_field"$1"_"$2".out #capture output into a file
    fi
done
